...
const CommandDialog = ({ children, ...props }) => {
  return (
    <Dialog {...props}>
      <DialogContent className="overflow-hidden p-0 py-2">
...
const CommandInput = React.forwardRef(({ className, wrapperClassName, ...props }, ref) => (
  <div className={cn('flex items-center border-b px-3', wrapperClassName)} cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        'flex h-10 w-full rounded-md bg-transparent py-3 text-sm outline-hidden border-0 ring-0 focus:ring-0 focus:outline-hidden focus-visible:ring-0 focus-visible:outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50',
        className
      )}
      {...props}
    />
