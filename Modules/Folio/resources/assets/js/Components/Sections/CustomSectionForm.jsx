 ...
 * Custom Section Form
 *
 * A single custom section with user-defined title and items.
 * Each item has: title, subtitle, date, location, description, url.
 */
...
function CustomItemRow({ control, basePath, index, onEdit, onRemove }) {
  const title = useWatch({ control, name: `${basePath}.${index}.title` });
  const subtitle = useWatch({ control, name: `${basePath}.${index}.subtitle` });
...
function CustomItemEditDialog({ open, onOpenChange, control, basePath, index }) {
  return (
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {index !== null && (
          <div className="space-y-4 px-6 py-4">
            <FormFieldRHF
              name={`${basePath}.${index}.title`}
              control={control}
              label="Title"
              placeholder="Item title"
            />
            <FormFieldRHF
              name={`${basePath}.${index}.subtitle`}
              control={control}
              label="Subtitle"
...
export default function CustomSectionForm({ control, sectionIndex }) {
  const [editIndex, setEditIndex] = useState(null);
  const basePath = `sections.custom.${sectionIndex}.items`;
