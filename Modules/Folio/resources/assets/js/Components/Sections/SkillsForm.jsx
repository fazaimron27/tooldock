...
function SkillItemRow({ control, index, onEdit, onRemove }) {
  const name = useWatch({ control, name: `sections.skills.items.${index}.name` });
  const keywords = useWatch({ control, name: `sections.skills.items.${index}.keywords` });
    ...
    <div className="group flex items-center gap-3 rounded-lg border bg-card px-4 py-3 transition-colors hover:bg-accent/50">
      <div className="flex flex-1 cursor-pointer items-center gap-3 min-w-0" onClick={onEdit}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium truncate">{name || 'Untitled Skill'}</p>
...
function SkillEditDialog({ open, onOpenChange, control, index, isNew }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader className="px-6 pt-6 pb-2">
          ...
          <DialogDescription>
            {isNew
              ? 'Add a skill category and its related keywords.'
              : 'Update the skill name and keywords.'}
          </DialogDescription>
        </DialogHeader>
        {index !== null && (
          <div className="space-y-4 px-6 py-4">
            <FormFieldRHF
              name={`sections.skills.items.${index}.name`}
              control={control}
              label="Skill Name"
              placeholder="JavaScript"
            />
            <FormFieldRHF
              name={`sections.skills.items.${index}.keywords`}
              control={control}
...
export default function SkillsForm({ control }) {
